...
def nsfw_img2img(
    ...
    future = job_store._executor.submit(
        ...
        strength if strength is not None else 0.6,
    )
    future.result()
...
def _run_flux_nsfw_chain(job_id, prompt, negative_prompt, height, width, flux_steps, nsfw_steps, guidance_scale, strength):
    try:
        ...
        # Step 2: NSFW img2img refine
        nsfw_h = (height // 8) * 8
        nsfw_w = (width // 8) * 8
        init_image = flux_image.resize((nsfw_w, nsfw_h))
...
def nsfw_chain(req: ChainRequest, request: Request):
    """FLUX txt2img → NSFW img2img. FLUX untuk kualitas dasar, NSFW untuk style refine."""
    job_id = uuid.uuid4().hex
    ...
    future = job_store._executor.submit(
        ...
        req.width or 1024,
        req.flux_steps or 8,
        req.nsfw_steps or 20,
        req.guidance_scale if req.guidance_scale is not None else 7.5,
        req.strength if req.strength is not None else 0.6,
    )
    future.result()
